...
//이미지
var h_souji = Paper.image('img/h_souji.png', 0, 0, 110, 100).toDefs();
...
// 단
bottomArea.circle(105, 80, 20).click(hadlerSa).attr({
  'fill': '#FCF5D4',
  'cursor': 'pointer'
});
bottomArea.text(97, 87, 'さ').click(hadlerSa).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.text(167, 270, 'そ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_17_so.wav');
  audio1.play();
}
...
h_souji.use().transform('t135 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(180, 550, ['そ', 'うじ', '청소']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_17_souji.wav');
  audio2.play();
}
