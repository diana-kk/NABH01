...
//이미지
var h_ie = Paper.image('img/h_ie.png', 0, 0, 100, 70).toDefs();
...
bottomArea.text(171, 270, 'い').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_04_ii.wav');
  audio1.play();
}
...
h_ie.use().transform('t135, 425').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['い', 'え', '집']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_04_ie.wav');
  audio2.play();
}
