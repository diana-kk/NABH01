...
//이미지
var h_ushi = Paper.image('img/h_ushi.jpg', 0, 0, 110, 90).toDefs();
...
// 단
bottomArea.circle(105, 80, 20).click(handlerAa).attr({
  'fill': '#FCF5D4',
  'cursor': 'pointer'
});
bottomArea.text(97, 87, 'あ').click(handlerAa).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.circle(317, 80, 20).click(hadlerOo).attr({
  'fill': '#FCF5D4',
  'cursor': 'pointer'
});
bottomArea.text(309, 87, 'お').click(hadlerOo).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
function hadlerOo() {
  location.replace('h_07_oo.html');
}
...
bottomArea.text(171, 270, 'う').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_05_uu.wav');
  audio1.play();
}
...
h_ushi.use().transform('t135, 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['う', 'し', '소']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_05_ushi.wav');
  audio2.play();
}
