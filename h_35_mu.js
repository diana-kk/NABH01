...
//이미지
var h_mushi = Paper.image('img/h_mushi.png', 0, 0, 110, 90).toDefs();
...
bottomArea.circle(158, 80, 20).click(hadlerMi).attr({
  'fill': '#FCF5D4',
  'cursor': 'pointer'
});
bottomArea.text(150, 87, 'み').click(hadlerMi).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 270, 'む').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_35_mu.wav');
  audio1.play();
}
...
h_mushi.use().transform('t130 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['む', 'し', '벌레']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_35_mushi.wav');
  audio2.play();
}
