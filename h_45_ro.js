...
//이미지
var h_roba = Paper.image('img/h_roba.png', 0, 0, 120, 90).toDefs();
...
bottomArea.text(171, 270, 'ろ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_45_ro.wav');
  audio1.play();
}
...
h_roba.use().transform('t130 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(186, 550, ['ろ', 'ば', '당나귀']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_45_roba.wav');
  audio2.play();
}
