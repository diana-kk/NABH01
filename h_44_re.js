...
//이미지
var h_rennga = Paper.image('img/h_rennga.jpg', 0, 0, 110, 90).toDefs();
...
// 단
bottomArea.circle(105, 80, 20).click(handlerRa).attr({
  'fill': '#FCF5D4',
  'cursor': 'pointer'
});
bottomArea.text(97, 87, 'ら').click(handlerRa).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 270, 'れ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_44_re.wav');
  audio1.play();
}
...
h_rennga.use().transform('t130 425').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(180, 550, ['れ', 'んが', '벽돌']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_44_rennga.wav');
  audio2.play();
}
