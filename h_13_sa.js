...
//이미지
var h_saji = Paper.image('img/h_saji.jpg', 0, 0, 110, 80).toDefs();
...
function handlerAft() {
  location.replace('h_14_shi.html');
}
...
topArea.text(22, 187, 'さ').click(hanlderLine).attr({
  'font-size': 20,
  'font-weight': 'bold',
  'cursor': 'pointer'
});
...
bottomArea.text(97, 87, 'さ').attr({
  'font-size': 20,
  'font-weight': 'bold'
});

bottomArea.circle(158, 80, 20).click(hadlershi).attr({
  'fill': '#FCF5D4',
  'cursor': 'pointer'
});
bottomArea.text(150, 87, 'し').click(hadlershi).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.circle(211, 80, 20).click(hadlerSu).attr({
  'fill': '#FCF5D4',
  'cursor': 'pointer'
});
bottomArea.text(203, 87, 'す').click(hadlerSu).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
function hadlerSu() {
  location.replace('h_15_su.html');
}

bottomArea.circle(264, 80, 20).click(hadlerSe).attr({
  'fill': '#FCF5D4',
  'cursor': 'pointer'
});
bottomArea.text(256, 87, 'せ').click(hadlerSe).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
function hadlerSe() {
  location.replace('h_16_se.html');
}

bottomArea.circle(317, 80, 20).click(hadlerSo).attr({
  'fill': '#FCF5D4',
  'cursor': 'pointer'
});
bottomArea.text(309, 87, 'そ').click(hadlerSo).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
function hadlerSo() {
  location.replace('h_17_so.html');
}
...
bottomArea.text(171, 270, 'さ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_13_sa.wav');
  audio1.play();
}
...
h_saji.use().transform('t135 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(185, 550, ['さ', 'じ', '숟가락']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_13_saji.wav');
  audio2.play();
}
