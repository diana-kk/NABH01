...
//이미지
var h_te = Paper.image('img/h_te.jpg', 0, 0, 100, 100).toDefs();
...
bottomArea.text(171, 270, 'て').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_21_te.wav');
  audio1.play();
}
...
h_te.use().transform('t135 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(210, 550, ['て', '손']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_21_te_voca.wav');
  audio2.play();
}
