...
//이미지
var h_kega = Paper.image('img/h_kega.jpg', 0, 0, 90, 90).toDefs();
...
bottomArea.text(171, 270, 'け').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_11_ke.wav');
  audio1.play();
}
...
h_kega.use().transform('t140, 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['け', 'が', '상처']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_11_kega.wav');
  audio2.play();
}
