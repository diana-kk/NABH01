...
//이미지
var h_nuno = Paper.image('img/h_nuno.png', 0, 0, 110, 90).toDefs();
...
bottomArea.text(171, 270, 'ぬ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_25_nu.wav');
  audio1.play();
}
...
h_nuno.use().transform('t135 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['ぬ', 'の', '천']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_25_nuno.wav');
  audio2.play();
}
