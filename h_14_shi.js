...
//이미지
var h_sio = Paper.image('img/h_sio.jpg', 0, 0, 110, 80).toDefs();
...
bottomArea.text(171, 270, 'し').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_14_shi.wav');
  audio1.play();
}
...
h_sio.use().transform('t135 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['し', 'お', '소금']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_14_sio.wav');
  audio2.play();
}
