...
//이미지
var h_rusu = Paper.image('img/h_rusu.jpg', 0, 0, 90, 90).toDefs();
...
bottomArea.text(171, 270, 'る').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_43_ru.wav');
  audio1.play();
}
...
h_rusu.use().transform('t130 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(186, 550, ['る', 'す', '부재중']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_43_rusu.wav');
  audio2.play();
}
