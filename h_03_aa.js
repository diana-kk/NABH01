...
//이미지
var h_aka = Paper.image('img/h_aka.png', 0, 0, 100, 70).toDefs();
...
bottomArea.text(171, 270, 'あ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_03_aa.wav');
  audio1.play();
}
...
h_aka.use().transform('t130, 430').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['あ', 'か', '빨강']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_03_aka.wav');
  audio2.play();
}
