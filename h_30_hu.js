...
//이미지
var h_huku = Paper.image('img/h_huku.jpg', 0, 0, 110, 90).toDefs();
...
bottomArea.text(171, 270, 'ふ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_30_hu.wav');
  audio1.play();
}
...
h_huku.use().transform('t140 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['ふ', 'く', '옷']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_30_huku.wav');
  audio2.play();
}
