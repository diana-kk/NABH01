...
//이미지
var h_hoshi = Paper.image('img/h_hoshi.jpg', 0, 0, 95, 95).toDefs();
...
// 단
bottomArea.circle(105, 80, 20).click(hadlerHa).attr({
  'fill': '#FCF5D4',
  'cursor': 'pointer'
});
bottomArea.text(97, 87, 'は').click(hadlerHa).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 270, 'ほ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_32_ho.wav');
  audio1.play();
}
...
h_hoshi.use().transform('t140 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['ほ', 'し', '별']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_32_hoshi.wav');
  audio2.play();
}
