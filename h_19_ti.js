...
//이미지
var h_titi = Paper.image('img/h_titi.png', 0, 0, 90, 100).toDefs();
...
bottomArea.text(171, 270, 'ち').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_19_ti.wav');
  audio1.play();
}
...
h_titi.use().transform('t135 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['ち', 'ち', '아빠']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_19_titi.wav');
  audio2.play();
}
