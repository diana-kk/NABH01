...
//이미지
var h_yuki = Paper.image('img/h_yuki.jpg', 0, 0, 110, 90).toDefs();
...
bottomArea.text(171, 270, 'ゆ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_39_yu.wav');
  audio1.play();
}
...
h_yuki.use().transform('t130 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['ゆ', 'き', '눈']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_39_yuki.wav');
  audio2.play();
}
