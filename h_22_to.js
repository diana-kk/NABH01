...
//이미지
var h_tokei = Paper.image('img/h_tokei.png', 0, 0, 100, 95).toDefs();
...
// 단
bottomArea.circle(105, 80, 20).click(hadlerTa).attr({
  'fill': '#FCF5D4',
  'cursor': 'pointer'
});
bottomArea.text(97, 87, 'た').click(hadlerTa).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 270, 'と').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_22_to.wav');
  audio1.play();
}
...
h_tokei.use().transform('t135 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(180, 550, ['と', 'けい', '시계']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_22_tokei.wav');
  audio2.play();
}
