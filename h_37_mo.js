...
//이미지
var h_mochi = Paper.image('img/h_mochi.jpg', 0, 0, 110, 80).toDefs();
...
bottomArea.text(171, 270, 'も').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_37_mo.wav');
  audio1.play();
}
...
h_mochi.use().transform('t135 430').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['も', 'ち', '떡']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_37_moti.wav');
  audio2.play();
}
