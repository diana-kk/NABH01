...
//이미지
var h_risu = Paper.image('img/h_risu.jpg', 0, 0, 90, 100).toDefs();
...
bottomArea.text(171, 270, 'り').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_42_ri.wav');
  audio1.play();
}
...
h_risu.use().transform('t130 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(186, 550, ['り', 'す', '다람쥐']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_42_risu.wav');
  audio2.play();
}
