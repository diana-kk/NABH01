...
//이미지
var h_koi = Paper.image('img/h_koi.png', 0, 0, 140, 80).toDefs();
...
bottomArea.text(171, 270, 'こ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_12_ko.wav');
  audio1.play();
}
...
h_koi.use().transform('t125, 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['こ', 'い', '잉어']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_12_koi.wav');
  audio2.play();
}
