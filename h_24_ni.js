...
//이미지
var h_nizi = Paper.image('img/h_nizi.png', 0, 0, 110, 90).toDefs();
...
bottomArea.text(171, 270, 'に').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_24_ni.wav');
  audio1.play();
}
...
h_nizi.use().transform('t130 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(185, 550, ['に', 'じ', '무지개']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_24_niji.wav');
  audio2.play();
}
