...
//이미지
var h_ki = Paper.image('img/h_ki.png', 0, 0, 90, 105).toDefs();
...
// 단
bottomArea.circle(105, 80, 20).click(hadlerKa).attr({
  'fill': '#FCF5D4',
  'cursor': 'pointer'
});
bottomArea.text(97, 87, 'か').click(hadlerKa).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.text(150, 87, 'き').attr({
  'font-size': 20,
  'font-weight': 'bold'
});

bottomArea.circle(211, 80, 20).click(hadlerKu).attr({
  'fill': '#FCF5D4',
  'cursor': 'pointer'
});
bottomArea.text(203, 87, 'く').click(hadlerKu).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 270, 'き').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_09_ki.wav');
  audio1.play();
}
...
h_ki.use().transform('t145, 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(210, 550, ['き', '나무']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_09_ki_voca.wav');
  audio2.play();
}
