...
bottomArea.text(177, 87, 'わ').click(handlerWa).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 270, 'を').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});
...
h_waru.use().transform('t130 410').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(170, 525, ['りんご', 'を', 'わる', '사과', '를', '쪼개다']).click(playAud02).attr({
  'font-size': 25,
  'fill': 'gray',
  'cursor': 'pointer'
});
