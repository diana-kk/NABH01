...
//이미지
var h_yoru = Paper.image('img/h_yoru.jpg', 0, 0, 110, 90).toDefs();
...
bottomArea.text(148, 87, 'や').click(hadlerYa).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 270, 'よ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_40_yo.wav');
  audio1.play();
}
...
h_yoru.use().transform('t130 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['よ', 'る', '밤']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_40_yoru.wav');
  audio2.play();
}
