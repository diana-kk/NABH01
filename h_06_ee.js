...
//이미지
var h_ee = Paper.image('img/h_ee.jpg', 0, 0, 90, 90).toDefs();
...
bottomArea.text(171, 270, 'え').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_06_ee.wav');
  audio1.play();
}
...
h_ee.use().transform('t135, 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(210, 550, ['え', '그림']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_06_ee_voca.wav');
  audio2.play();
}
