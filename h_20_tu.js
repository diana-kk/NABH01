...
//이미지
var h_tuti = Paper.image('img/h_tuti.jpg', 0, 0, 110, 100).toDefs();
...
bottomArea.text(171, 270, 'つ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_20_tu.wav');
  audio1.play();
}
...
h_tuti.use().transform('t135 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['つ', 'ち', '흙']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_20_tuti.wav');
  audio2.play();
}
