...
//이미지
var h_seki = Paper.image('img/h_seki.png', 0, 0, 110, 80).toDefs();
...
bottomArea.text(171, 270, 'せ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_16_se.wav');
  audio1.play();
}
...
h_seki.use().transform('t135 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['せ', 'き', '좌석']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_16_seki.wav');
  audio2.play();
}
