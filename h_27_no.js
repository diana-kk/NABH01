...
//이미지
var h_nori = Paper.image('img/h_nori.png', 0, 0, 105, 90).toDefs();
...
// 단
bottomArea.circle(105, 80, 20).click(hadlerNa).attr({
  'fill': '#FCF5D4',
  'cursor': 'pointer'
});
bottomArea.text(97, 87, 'な').click(hadlerNa).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 270, 'の').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_27_no.wav');
  audio1.play();
}
...
h_nori.use().transform('t135 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['の', 'り', '김']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_27_nori.wav');
  audio2.play();
}
