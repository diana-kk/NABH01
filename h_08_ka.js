...
//이미지
var h_kaki = Paper.image('img/h_kaki.jpg', 0, 0, 90, 85).toDefs();
...
bottomArea.text(171, 270, 'か').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_08_ka.wav');
  audio1.play();
}
...
h_kaki.use().transform('t150, 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['か', 'き', '감']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_08_kaki.wav');
  audio2.play();
}
