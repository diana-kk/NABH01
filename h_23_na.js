...
//이미지
var h_nashi = Paper.image('img/h_nashi.png', 0, 0, 90, 90).toDefs();
...
bottomArea.text(97, 87, 'な').attr({
  'font-size': 20,
  'font-weight': 'bold'
});

bottomArea.circle(158, 80, 20).click(hadlerNi).attr({
  'fill': '#FCF5D4',
  'cursor': 'pointer'
});
bottomArea.text(150, 87, 'に').click(hadlerNi).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.circle(211, 80, 20).click(hadlerNu).attr({
  'fill': '#FCF5D4',
  'cursor': 'pointer'
});
bottomArea.text(203, 87, 'ぬ').click(hadlerNu).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
function hadlerNu() {
  location.replace('h_25_nu.html');
}

bottomArea.circle(264, 80, 20).click(hadlerNe).attr({
  'fill': '#FCF5D4',
  'cursor': 'pointer'
});
bottomArea.text(256, 87, 'ね').click(hadlerNe).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
function hadlerNe() {
  location.replace('h_26_ne.html');
}

bottomArea.circle(317, 80, 20).click(hadlerNo).attr({
  'fill': '#FCF5D4',
  'cursor': 'pointer'
});
bottomArea.text(309, 87, 'の').click(hadlerNo).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 270, 'な').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_23_na.wav');
  audio1.play();
}
...
h_nashi.use().transform('t145 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['な', 'し', '배']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_23_nasi.wav');
  audio2.play();
}
