...
//이미지
var h_sushi = Paper.image('img/h_sushi.png', 0, 0, 110, 80).toDefs();
...
bottomArea.text(171, 270, 'す').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_15_su.wav');
  audio1.play();
}
...
h_sushi.use().transform('t135 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['す', 'し', '초밥']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_15_susi.wav');
  audio2.play();
}
