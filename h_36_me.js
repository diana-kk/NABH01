...
//이미지
var h_me = Paper.image('img/h_me.jpg', 0, 0, 100, 80).toDefs();
...
bottomArea.text(171, 270, 'め').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_36_me.wav');
  audio1.play();
}
...
h_me.use().transform('t135 430').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(210, 550, ['め', '싹']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_36_me_voca.wav');
  audio2.play();
}
