...
//이미지
var h_heso = Paper.image('img/h_heso.png', 0, 0, 75, 115).toDefs();
...
bottomArea.text(171, 270, 'へ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_31_he.wav');
  audio1.play();
}
...
h_heso.use().transform('t140 410').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['へ', 'そ', '배꼽']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_31_heso.wav');
  audio2.play();
}
